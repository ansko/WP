...
def pressure_layers(stress_dump):
    ...
    for i in range(1, systemsize + 1):
        ...
        # mixed --------------------------------------------------------
        #elif (17.8 > stress_dump[i][0] > 12.6):
        #    press1 += stress_dump[i][1] 
        #if(17.8 < stress_dump[i][0] < 22.3):
        #    press2 += stress_dump[i][1]
        #elif (22.3 < stress_dump[i][0] < 27):
        #    press3 += stress_dump[i][1]
        #elif (27 < stress_dump[i][0] < 31.8):
        #    press4 += stress_dump[i][1]
        #elif (31.8 < stress_dump[i][0] < 36.2):
        #    press5 += stress_dump[i][1]
        #elif (36.2 < stress_dump[i][0] < 40.8):
        #    press6 += stress_dump[i][1]
        #elif (40.8 < stress_dump[i][0] < 46):
    ...
    # segregated--------------------------------------------------------
        #elif (64.8 > stress_dump[i][0] > 59.5):
        #    press1 += stress_dump[i][1] 
        #if(64.8 < stress_dump[i][0] < 69.3):
        #    press2 += stress_dump[i][1]
        #elif (69.3 < stress_dump[i][0] < 74):
        #    press3 += stress_dump[i][1]
        #elif (74 < stress_dump[i][0] < 78.2):
        #    press4 += stress_dump[i][1]
        #elif (78.2 < stress_dump[i][0] < 82.7):
        #    press5 += stress_dump[i][1]
        #elif (82.7 < stress_dump[i][0] < 87.3):
        #    press6 += stress_dump[i][1]
        #elif (87.3 < stress_dump[i][0] < 92.5):
        #    press7 += stress_dump[i][1]       
    ...
    #        press7 / (lx * ly * 5)]
        ...
        if(stress_dump[i][0] < 1 or stress_dump[i][0] > 38):
            press4 += stress_dump[i][1]
        elif (stress_dump[i][0] < 5.5):
            press5 += stress_dump[i][1]
        elif (stress_dump[i][0] < 10):
            press6 += stress_dump[i][1]
        elif (stress_dump[i][0] < 15):
            press7 += stress_dump[i][1]
        elif (24 < stress_dump[i][0] < 29):
            press1 += stress_dump[i][1]
...
def main(atomsnum=systemsize, step=100):
    folder = '/home/anton/WorkingPrograms/10ch_slow_cooling/Dumps/' #---------------------------------------------------------------------------folder
